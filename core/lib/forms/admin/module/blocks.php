...
if (is_numeric($_POST['id'])) {
	...
	$template = file_get_contents(resolve("/templates/page_types/".$module['template'].".php"));
	...
	<form id="page_blocks" method="POST" action="" enctype="multipart/form-data" style="margin:10px">
    	<?php if (empty($_POST['blocktype'])) {  // check if should be loading specific block type ?>
            <select name="block_type" onchange="block.getBlockForm(<?=$_POST['id']; ?>, this.value, 'module', <?=(!empty($_POST['content_id']))?$_POST['content_id']:0; ?>)" style='width:200px;'>
        ...
        <input type="hidden" name="content_id" value="<?=$_POST['content_id']; ?>" />
        <input type="hidden" name="page_type" value="module" />
        
	</form>
    <script type='text/javascript'>
		<?php if (!empty($_POST['blocktype'])) { // check if should be loading specific block type ?>
			block.getBlockForm(<?=$_POST['id']?>, '<?=$_POST['blocktype']?>', 'module');
		<?php } ?>
	</script>
	<?php 
}