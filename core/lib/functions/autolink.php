<?php
	...
	function text_parse($text) {
		$output = autolink_email(
					autolink(
						str_replace("<p >&nbsp;</p>", "", 
							preg_replace('/class=".*?"/', '', 
								preg_replace('/style=".*?"/', '', 
									strip_tags($text, "<p><em><i><b><strong>")
								)
							)
						)
					)
	...
	function checklink($link) {
		return (strpos($link, "http://")===false && strpos($link, "https://")===false) ? "http://".$link : $link ;
	}
	
	function autolink($text, $limit=30, $tagfill='target="_blank"'){
		
		$text = str_replace('www.', ' www.', $text);
		$text = str_replace('http:// www.', ' http://www.', $text);
