...
class Paginator {
	...
	***********************************/
	function createPagination($elements) {
		...
		$page_links = "<a href='javascript:void(0)' onclick=\"$('.".$paginator_id."_pages').css('display','block');\" class='pagination_link pagination_all'>ALL</a>\n | ";
		
		if ($this->type == 'count') {
			
			if ($this->pagination_type == 'standard') {
				for($i=0; $i<=$pages; $i++) {
					$page_links .= "<a href='javascript:void(0)' onclick=\"pagination('".$paginator_id."', 'numeric', {letter:'".$i."'});\" class='pagination_link'>".($i+1)."</a>\n | ";
				}
				$page_links = rtrim($page_links, " | ");
			} else {
				$page_links .= "<a href='javascript:void(0)' onclick=\"pagination('".$paginator_id."', 'numeric', {direction:'prev'});\" class='pagination_link pagination_prev'><span>previous page</span></a>\n";
				$page_links .= "<a href='javascript:void(0)' onclick=\"pagination('".$paginator_id."', 'numeric', {direction:'next'});\" class='pagination_link pagination_next'><span>next page</span></a>\n";
			...
			foreach ($alpha as $letter) {
				$page_links .= "<a href='javascript:void(0)' onclick=\"pagination('".$paginator_id."', 'alpha', {letter:'".$letter."'});\">".$letter."</a>\n | ";
			}
			$page_links = rtrim($page_links, " | ");
			...
			do {	
				...
				$caloutput .= "\n\t<li><a onclick=\"pagination('".$paginator_id."', 'date', {year:'".$y."'})\">".$y."</a><ul>";
				...
				if (!$by_year) {
					...
					for ($m=1; $m<=12; $m++) {
						...
						if ($do_month == true) {
							...
							$caloutput .= "\n\t\t<li><a onclick=\"pagination('".$paginator_id."', 'date', {year:'".$y."',month:'".$m."'})\">".date('F', strtotime($y."-".$m."-01"))."</a></li>";
		...
		if (!empty($_GET['p'])) {
			...
			$(document).ready(function() { pagination('<?=$paginator_id?>', '<?=$this->type?>', {letter:'<?=$_GET['p']?>'}); });
			</script>            
			<?php
			$output .= ob_get_clean();
		}
		if (!empty($_GET['date'])) {
			...
			?>
			<script type='text/javascript'>
			$(document).ready(function() { pagination('<?=$paginator_id?>', '<?=$this->type?>', {<?=$settings?>}); });
			</script>            
			<?php
			$output .= ob_get_clean();
		}
		if (count($elements)) {
			return "<div class='pagination_block'>".$output."</div>";
		} else {
