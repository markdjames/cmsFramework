...
if (is_numeric($_POST['id'])) {
	$block = $b->getBlock($_POST['id'], 'files', $_POST['type'], $_POST['content']);
	...
	?>
    <h4 style='clear:both'>Files Block</h4>
    <label>Title<br />
    <input type="text" class='require_onced' name="block[title]" value="<?=$block['title']?>" /></label>
    
    <div class='upload_form'>
    	<input style='float:right' type="button" onclick="($('#image_file').val()!='')?uploadFile('assets/files/<?=$_POST['id']?>', addFiles()):alert('please select a file to upload');" value="Upload" />
        <label for="fileToUpload">Select a File to Upload<br />
        <input type="file" name="files_file" id="files_file" onchange="uploadFileSelected(this.id, new Array('pdf','jpg','png','zip'));"/></label>
    ...
    <div id='gallery_images'>
		...
		if (count($files)) {
		    foreach($files as $k=>$file) {
                ...
                <label style='clear:left'>Caption<br /><input type="text" name="block[files][<?=$k?>][caption]" value="<?=$file['caption']?>" /></label>
                <label >Description<br /><input type="text" name="block[files][<?=$k?>][description]" value="<?=$file['description']?>" /></label>
                <div style='clear:both'></div>
                </div>
                <?php				
			}
    ...
    <input type="hidden" name="block_name" value="files" />
    <input type="button" style='float:right' onclick="checkForm('page_blocks')" value="<?=(empty($block['id'])) ? "Add Block" : "Update Block"?>" />
   	<?php 
	if (!empty($block['id'])) {
		?>
    	<input type="button" onclick="deleteRecord(<?=$block['id']?>, 'block_files', {type:'blocks',category:'files'})" value='Delete' />
		<?php
	}
	?>
